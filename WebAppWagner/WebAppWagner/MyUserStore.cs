            ...
            using (var connection = GetOpenConnection())
            {
                await connection.ExecuteAsync
                    (
                        ...
                        "[NormalizedUserName]," +
                        "[PasswordHash])" +
                        "Values(@id,@username,@normalizedUserName,@passwordHasher)",
                        new
                        {
                            id = user.Id,
...
(@"Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=IdentityCurso;Data Source=WAGNERBARROS\SQLEXPRESS");
            connection.Open();
            return connection;
        }
